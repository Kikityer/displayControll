...
/**
 * @ClassName ControlFrame6ser1
 * @Description TODO   服务器4   控制帧结构的组帧（同时结束发送和结束接收）用于收发模式关闭时
 * @Author Kikityer
 * @Date 2019/3/21 13:59
 * @Version 1.0.0
 **/
public class ControlFrame6ser4 {
    /**
    ...
    static byte[] desId = {0x00,0x04};   //显控向服务器发 1表示一号服务器 ，2表示二服务器 。。  2字节
    int time;       //当前时刻到当天凌晨的毫秒数     4字节
    static byte[] label = new byte[]{0x00,0x02};  //数据帧标志位   2字节

    ToByte toByte = new ToByte();

    public byte[][] controllPack6() throws ParseException {
        ...
        byte[] data = new byte[]{0x00,0x12};    //停止发送和停止接收帧结构
        byte sum = 0;
